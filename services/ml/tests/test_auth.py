...
# GET /ml-preview and POST /fill-up-history (see app/main.py). Both
# endpoints used to be reachable with no auth at all, letting anyone
# pass an arbitrary ?user_id= and read that user's fill-up history; this
# checks the fix actually rejects unauthenticated/incorrectly-authenticated
# requests and still serves correctly-authenticated ones.
